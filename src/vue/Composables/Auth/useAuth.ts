...
import { Axios } from "./../../Composables/Fetchers/BaseAxios";
...
export async function useAuth() {
  ...
  async function validateSession() {
    const encrypted_lensix_web_session: string = (await localforage.getItem("lensix_web_session")) ?? "";
  ...
  async function setSession(lensix_web_session: string) {
    ...
    await localforage.setItem("lensix_web_session", encrypted_lensix_web_session);
    let minutes = 60;
    const now = new Date();
